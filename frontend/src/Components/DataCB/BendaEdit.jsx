...
export default function BendaEdit() {
  ...
  useEffect(() => {
    const getBendaID = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/benda/${id}`);
  ...
  const updateBenda = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.patch(`http://localhost:5000/benda/${id}`, {
        nama,
        namalain,
        provinsi,
        kota,
  ...
  return (
    <div className="font-romans text-secondary py-3 px-2">
      ...
      <div>
        <form onSubmit={updateBenda} className="w-full mb-10">
